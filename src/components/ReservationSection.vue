<template>
  <section id="reservation" class="relative py-24 md:py-32 px-6 md:px-16 overflow-hidden">
      ...
      <div class="bg-white/5 backdrop-blur-md rounded-2xl p-10 md:p-14 border border-white/5">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mb-6">
          ...
          <div>
            ...
            >
              ...
              <option value="10+" class="text-charcoal">{{ t('reservation.guestOptions.10+') }}</option>
            </select>
          </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            class="inline-flex items-center justify-center gap-2 bg-[#25D366] text-white font-heading font-semibold px-6 py-3 text-sm rounded-full transition-opacity duration-200 hover:opacity-90"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            class="inline-flex items-center justify-center gap-2 text-white font-heading font-semibold px-6 py-3 text-sm rounded-full transition-opacity duration-200 hover:opacity-90"
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            class="inline-flex items-center justify-center gap-2 bg-[#0084FF] text-white font-heading font-semibold px-6 py-3 text-sm rounded-full transition-opacity duration-200 hover:opacity-90"
          >
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.36 2 2 6.13 2 11.7c0 2.91 1.2 5.42 3.15 7.2.16.15.26.36.27.59l.05 1.85c.02.62.67 1.03 1.23.78l2.07-.91c.17-.08.37-.1.55-.06.9.25 1.87.38 2.88.38 5.64 0 10-4.13 10-9.7S17.64 2 12 2zm5.89 7.5l-2.89 4.58c-.46.73-1.44.91-2.13.39l-2.3-1.72a.72.72 0 00-.86 0l-3.1 2.35c-.41.31-.95-.18-.68-.62l2.89-4.58c.46-.73 1.44-.91 2.13-.39l2.3 1.72a.72.72 0 00.86 0l3.1-2.35c.41-.31.95.18.68.62z"/></svg>
            Messenger
          </a>
        </div>
...
const whatsappUrl = computed(() => {
  const text = encodeURIComponent(
    `Bonjour, je souhaite réserver chez Ilot Coco Beach.\nNom: ${form.name}\nTél: ${form.phone}\nDate: ${form.date}\nPersonnes: ${form.guests}\n${form.voucher ? 'Code promo: ' + form.voucher + '\n' : ''}${form.message ? 'Message: ' + form.message : ''}`
  )
