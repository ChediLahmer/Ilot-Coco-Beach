<template>
  <section
    ...
    class="bg-gradient-to-bl from-sand-dark/50 via-sand to-ocean-light/20 px-6 py-20 md:px-12 lg:px-20"
  >
    <div class="mx-auto max-w-6xl animate-pulse">
      ...
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div
          v-for="index in 3"
          :key="index"
          class="overflow-hidden rounded-lg border border-charcoal/8 bg-white"
        >
  ...
  >
    <div class="mx-auto max-w-6xl">
      ...
      <div
        ...
        class="mt-10 rounded-2xl border border-charcoal/8 bg-white/70 px-6 py-10 text-center shadow-sm"
      >
        <p class="text-sm text-charcoal/60">{{ t("menu.empty") }}</p>
      </div>
    </div>
  </section>
</template>
...
watch(
  menuCategories,
  (categories) => {
    if (!activeCategory.value && categories.length) {
