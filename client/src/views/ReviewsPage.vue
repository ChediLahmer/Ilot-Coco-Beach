<template>
  <div class="min-h-screen bg-sand">
    ...
    <section class="relative overflow-hidden px-6 pb-16 pt-[7.5rem] md:px-16 md:pb-20 md:pt-[8.5rem]">
      <div class="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,125,97,0.12),transparent_22%),radial-gradient(circle_at_bottom_right,rgba(29,166,179,0.14),transparent_28%)]" />
          ...
          <div>
            ...
            <div class="mt-10 grid gap-4 sm:grid-cols-2">
              <article class="premium-card rounded-[1.75rem] p-6">
                ...
                <p class="mt-2 text-sm text-charcoal/60">{{ reviews.length }} {{ t('reviews.reviewsLabel') }}</p>
              </article>
            ...
            <div class="premium-card mt-8 rounded-[1.75rem] p-6 sm:p-8">
              <p class="font-heading text-[0.68rem] font-bold uppercase tracking-[0.22em] text-coral/80">
                {{ copy.formTitle }}
              </p>
              ...
              <form class="mt-6 space-y-5" @submit.prevent="submitReview">
                <div>
                  <label class="mb-3 block text-xs font-heading font-bold uppercase tracking-[0.18em] text-charcoal/48">
                    {{ copy.nameLabel }}
                  </label>
                  <input v-model="form.userName" type="text" :placeholder="copy.namePlaceholder" class="booking-field" />
                </div>

                <div>
                  <p class="mb-3 block text-xs font-heading font-bold uppercase tracking-[0.18em] text-charcoal/48">
                    {{ copy.ratingLabel }}
                  </p>
                  <div class="flex items-center gap-2">
                    <button
                      ...
                      :class="star <= form.rating ? 'text-coral' : 'text-charcoal/18'"
                ...
                <div>
                  ...
                  <textarea v-model="form.comment" rows="5" :placeholder="copy.commentPlaceholder" class="booking-field resize-none" />
                </div>

                <div class="flex flex-wrap items-center gap-4">
                  <button
                    type="submit"
                    class="inline-flex items-center justify-center rounded-full bg-[linear-gradient(135deg,var(--color-coral),var(--color-gold))] px-6 py-3 font-heading text-[0.76rem] font-bold uppercase tracking-[0.16em] text-white shadow-[0_16px_34px_rgba(255,125,97,0.22)]"
                  >
                    {{ copy.submit }}
                  </button>
...
const copy = computed(() => ({
  fr: {
    ...
    subtitle: 'Consultez les retours clients et partagez votre propre expérience de Coco Beach.',
    formTitle: 'Laisser un avis',
    formSubtitle: 'Ajoutez votre note et quelques mots pour aider les prochains visiteurs.',
    nameLabel: 'Votre nom',
    namePlaceholder: 'Entrez votre prénom ou nom',
    ratingLabel: 'Votre note',
    commentLabel: 'Votre avis',
    commentPlaceholder: 'Racontez votre expérience, votre table préférée ou votre plat marquant…',
  ...
  en: {
    title: 'Guest reviews',
    subtitle: 'Browse recent guest feedback and share your own Coco Beach experience.',
    ...
    commentPlaceholder: 'Tell us about your experience, favorite spot or signature dish…',
  ...
  ar: {
    ...
    ratingLabel: 'التقييم',
    commentLabel: 'رأيك',
    commentPlaceholder: 'احكِ لنا عن تجربتك أو طاولتك المفضلة أو طبقك المفضل…',
    submit: 'نشر التقييم',
